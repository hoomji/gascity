...
// openCodePluginPackPath is the embedded OpenCode plugin the hooks installer
// materializes into {workDir}/.opencode/plugins/gascity.js.
const openCodePluginPackPath = "overlay/per-provider/opencode/.opencode/plugins/gascity.js"

// fakeGCDriver is a tiny executable `gc` stand-in. `prime --hook` returns a
// constant session context; `nudge drain --inject` returns a new current-time
// line on every call, exactly like the real consumptive hook; `mail check
// --inject` returns nothing. It is CommonJS (.cjs) so the staged plugin's
// package.json module type cannot reinterpret it.
const fakeGCDriver = `#!/usr/bin/env node
...
// openCodePromptDriver negotiates two consecutive turns against one plugin
// instance and prints the resulting system prompt, per-message system override,
// and user-message text parts as JSON.
const openCodePromptDriver = `import { pathToFileURL } from "node:url";
...
// TestOpenCodePluginKeepsSystemPromptByteStableAcrossTurns executes the
...
func TestOpenCodePluginKeepsSystemPromptByteStableAcrossTurns(t *testing.T) {
	nodeBin, err := exec.LookPath("node")
	if err != nil {
		t.Skip("node not installed; cannot execute the OpenCode plugin")
	}

	plugin, err := iofs.ReadFile(core.PackFS, openCodePluginPackPath)
	if err != nil {
	...
	// OpenCode loads workdir plugins as ES modules.
	...
	if err := os.WriteFile(driverPath, []byte(openCodePromptDriver), 0o644); err != nil {
		t.Fatalf("stage driver: %v", err)
	}

	gcDir := t.TempDir()
	gcPath := filepath.Join(gcDir, "fake-gc.cjs")
	if err := os.WriteFile(gcPath, []byte(fakeGCDriver), 0o755); err != nil {
	...
	var got struct {
		First  openCodeTurn `json:"first"`
		Second openCodeTurn `json:"second"`
	}
	...
	for i, turn := range []openCodeTurn{got.First, got.Second} {
		if len(turn.Parts) != 2 {
